...
class TrDRC(Step):
    ...
    def run(
        ...
        **kwargs,
        ...
        state = super().run()
        if (drc_errors_count := state.metrics.get("route__drc_errors")) is None:
            warn("No OpenROAD routing DRC errors reported. OpenROAD.DetailedRouting routing didn't run")
...
class MagicDRC(Step):
    ...
    def run(
        ...
        **kwargs,
        ...
        state = super().run()
        if (drc_errors_count := state.metrics.get("magic__drc_errors")) is None:
...
class WireLength(Step):
    ...
    def run(
        ...
        **kwargs,
        ...
        elif (max_wire_length := state.metrics.get("route__max__wirelength")) is None:
            warn("No max wirelength errors reported. Odb.ReportWireLength didn't run")
        elif max_wire_length > threshold:
            error_msg = (
                f"Max wire length ({max_wire_length}) is larger than the threshold {threshold}"
            )
            err(f"{error_msg} - deferred")
            raise DeferredStepError(error_msg)
        return state
...
class LVS(Step):
    ...
    def run(
        ...
        **kwargs,
        ...
        state = super().run()
        lvs_errors_count = state.metrics.get("lvs__total__errors")
