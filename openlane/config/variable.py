...
class Macro:
    ...
    nl: Union[Path, Paths] = field(default_factory=lambda: [])
    spef: Dict[str, Paths] = field(default_factory=lambda: {})
    lib: Dict[str, Paths] = field(default_factory=lambda: {})
    spice: Paths = field(default_factory=lambda: [])
    sdf: Dict[str, Paths] = field(default_factory=lambda: {})
    ...
    def from_state(Self, state: State) -> "Macro":
        kwargs = {
            "nl": state[DesignFormat.NETLIST],
            "spef": state[DesignFormat.SPEF],
        }
        for format in [DesignFormat.GDS, DesignFormat.LEF]:
            view = state[format]
            if view is None or len(view) == 0:
                raise ValueError(
                    f"State cannot be used to create Macro- no {format.name} views found"
                )
            kwargs[format.value.id] = view
