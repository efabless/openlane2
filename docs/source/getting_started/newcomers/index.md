...
`x` is multiplied by each bit of the serial input `y` as it is shifted in. The
product is generated and serially output on the wire `p`. Check
[this paper](https://doi.org/10.1109/ACSSC.1993.342534) for more information about the `SPM`.


#### RTL

The source {term}`RTL` of the design will consist of 2 files, which are `spm.v` and `pm32.v`.
...
1. Create the files `~/my_designs/pm32/pm32.v`, `~/my_designs/pm32/spm.v`, and add [RTL](#rtl) content to it.
