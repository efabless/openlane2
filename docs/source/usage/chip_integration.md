...
| `MACRO_PLACEMENT_CFG` | A path to a file containing a line-break delimited list of instances and positions if you want to manually place the macros in specific locations, in the format `instance_name X_pos Y_pos Orientation`. The `manual_macro_placement_test` design should be a good example. |
...
You will most likely need a custom Python-based flow to harden a complex, full chip. Caravel, for example, has one such flow.

First you need to harden the padframe as a separate macro.
...
| Key | Description |
|-|-|
| `SYNTH_FLAT_TOP` | `1/0` (Tcl), `true/false` (JSON): Flattens the padframe if true (and presented in a chip_io module). Otherwise, you can harden it separately.  |

The following inputs are provided to produce the final GDSII:

1. Padframe cfg file (provided by the user or generated by padring).
2. Hardened LEF & GDSII files for the core module, generated [here](#hardening-the-core)
3. Top level netlist instantiating pads and core module (Could be provided by the user or generated by topModuleGen)

A script for hardening I/Os does the following:
...
- Routes.
- Streams out the GDSII and the LEF views.
...
See [Hardening Macros](./hardening_macros.md).
...
[Advanced Power Grid Control](./advanced_power_grid_control.md) covers automating the power routing process at both the core and macro levels. Otherwise, refer to [the OpenROAD `pdngen` documentation](https://openroad.readthedocs.io/en/latest/main/src/pdn/README.html) for more details about the syntax. In case you needed to create your own `pdn.tcl` then point to it using the`PDN_CFG` configuration variable.
